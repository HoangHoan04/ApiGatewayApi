...
public class GatewayExceptionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            // Client chủ động ngắt kết nối (F5, navigate, abort request), không ghi log error
            _logger.LogDebug("[ApiGateway] Client đã đóng kết nối trước khi nhận phản hồi: {Path}", context.Request.Path);
        }
        catch (Exception ex)
        {
